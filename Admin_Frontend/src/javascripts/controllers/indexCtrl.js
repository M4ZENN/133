class IndexCtrl {
  ...
  getCatsSuccess(data) {
    ...
    data.forEach((cat) => {
      ...
      const breedName =
        cat.breed && cat.breed.name ? cat.breed.name.trim() : "Inconnu";
      $(catCard).find(".cat-info .info-field p").eq(1).text(breedName);
      $(catCard).find(".cat-info .info-field p").eq(2).text(cat.funFact);
      $(catCard)
        .find(".cat-info .info-field p")
        .eq(3)
        .text(cat.description.trim());

      const image =
        cat.image ||
        "https://coin-images.coingecko.com/coins/images/52603/large/OIALogo.jpg?1733756422";
